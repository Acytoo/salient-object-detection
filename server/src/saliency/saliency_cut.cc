...
saliencycut::SaliencyCut::SaliencyCut(const Mat& img3f)
...
saliencycut::SaliencyCut::~SaliencyCut(void) {
  if (_graph)
    delete _graph;
}
...
Mat saliencycut::SaliencyCut::CutObjs(const Mat& _img3f, const Mat& _sal1f,
...
void saliencycut::SaliencyCut::initialize(const Rect &rect) {
...
void saliencycut::SaliencyCut::initialize(const Mat& sal1u) {
...
void saliencycut::SaliencyCut::initialize(const cv::Mat& sal1f,
                                          float t1, float t2) {
...
void saliencycut::SaliencyCut::initGraph() {
...
int saliencycut::SaliencyCut::ExpandMask(const cv::Mat& fMask, cv::Mat& mask1u,
...
int saliencycut::SaliencyCut::updateHardSegmentation() {
  int changed = 0;
  for (int y = 0, id = 0; y < _h; ++y) {
    float* segVal = _segVal1f.ptr<float>(y);
    int* triMapD = _trimap1i.ptr<int>(y);
